...
import com.tcc.unisolidariamotorista.databinding.ActivityMapTripBinding
import com.tcc.unisolidariamotorista.fragments.ModalBottomSheetTripInfo
...
import com.tcc.unisolidariamotorista.models.Prices
...
import com.tcc.unisolidariamotorista.providers.ConfigProvider
...
//        binding.btnDisconnect.setOnClickListener { disconnectDriver() }
    ...
    private fun sendNotification(status: String) {
        ...
        map.put("title", "ESTADO DA VIAGEM")
        map.put("body", status)
        ...
        notificationProvider.sendNotification(body).enqueue(object: Callback<FCMResponse> {
            override fun onResponse(call: Call<FCMResponse>, response: Response<FCMResponse>) {
                if (response.body() != null) {
                    ...
                    Toast.makeText(this@MapTripActivity, "Ocorreu um erro ao enviar a notificação", Toast.LENGTH_LONG).show()
                }
            }
    ...
    private fun addOriginMarker(position: LatLng) {
        markerOrigin = googleMap?.addMarker(MarkerOptions().position(position).title("Pegue aqui")
            .icon(BitmapDescriptorFactory.fromResource(R.drawable.icons_location_person)))
    }

    private fun addDestinationMarker() {
        if (destinationLatLng != null) {
            markerDestination = googleMap?.addMarker(MarkerOptions().position(destinationLatLng!!).title("Pegue aqui")
    ...
    override fun onDestroy() { //FECHE O APLICATIVO OU VÁ PARA OUTRA ATIVIDADE
        super.onDestroy()
        easyWayLocation?.endUpdates()
